...
# CONFIG
# Channel/role options, logging channels, and leaderboard channels are configured
# per-deployment in config.toml under [channels] (see Config.channel_options etc.
# in utils.py) rather than hardcoded here.
...
class ChannelSelect(discord.ui.Select):
    def __init__(self, builder):
        self.builder = builder
        self.channel_options = builder.ctx.bot.config.channel_options
        options = [discord.SelectOption(label=name, value=str(cid)) for name, cid in self.channel_options]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.placeholder = next((name for name, id in self.channel_options if id == cid), "Select Channel")
...
class PingSelect(discord.ui.Select):
    def __init__(self, builder):
        self.builder = builder
        self.ping_role_options = builder.ctx.bot.config.ping_role_options
        options = [discord.SelectOption(label=name, value=str(rid)) for name, rid in self.ping_role_options]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        self.placeholder = next((name for name, id in self.ping_role_options if id == rid), "Select Ping Role")
        await interaction.response.edit_message(embed=self.builder.announcement.embed(), view=self.builder.view)
...
class PostButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for cid in self.builder.ctx.bot.config.logging_channel_ids:
            ch = self.builder.ctx.guild.get_channel(cid)
            if ch:
                await ch.send(embed=ann.embed())

        if ann.channel.id in self.builder.ctx.bot.config.leaderboard_channel_ids:
